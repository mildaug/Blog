...
# Add topics
topics = []
topics.append(Topics(name='Sodininkyste'))
topics.append(Topics(name='Gamtos mokslai'))
topics.append(Topics(name='Maisto gaminimas'))
topics.append(Topics(name='Technologijos'))
...
posts.append(Posts(user_id=1, date=date(2023, 5, 12), topic_id=4, name='Sodininkyste pradedantiems', content='Jeigu neseniai įsigijote namą arba butą su sodu arba sodinių, pirmą sezoną verčiau neskubėti. Po juoda žemes galite atrasti buvusio šeimininko sodo svajones. Daugelis augalų yra daugiamečiai ir žydi kiekvienais metais.'))
posts.append(Posts(user_id=3, date=date(2023, 5, 15), topic_id=1, name='Kas yra ChatGPT?', content='ChatGPT yra pokalbių roboto programa, kuri atsakinėja į tekstinius pranešimus. Tai yra OpenAI sukurtas didelis kalbos modelis, kuris yra mokomas analizuoti ir generuoti tekstą įvairiuose kontekstuose.'))
